...
set_gdm_autologin() {
    ...
    if [[ -z $config_file ]]; then
        echo "Unsupported system. Cannot determine gdm config file location." >&2
        return 1
    fi

    mkdir -p "$config_dir"
    rm -f "$config_dir/.rsetup-autologin"

    if [[ ! -f $config_file ]]; then
        if [[ "$switch" == "ON" ]]; then
            echo "gdm is not installed. Auto login will be postponed." >&2
            echo "$user" > "$config_dir/.rsetup-autologin"
        fi
        return 0
    fi
...
set_suspend_status() {
    ...
    OFF)
        ...
        ;;
    *)
        echo "Invalid options: $switch (use ON/OFF)" >&2
        return 1
        ;;
    esac
